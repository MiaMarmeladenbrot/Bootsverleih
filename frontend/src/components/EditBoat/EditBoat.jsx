...
const EditBoat = ({ boat }) => {
  ...
  const [name, setName] = useState(boat.boatName);
  const [typ, setTyp] = useState(boat.boatType);
  const [material, setMaterial] = useState(boat.material);
  const [baujahr, setBaujahr] = useState(boat.constructionYear);
  const [seriennummer, setSeriennummer] = useState(boat.serialNumber);
  ...
  return (
    <div>
      ...
      <form className={showEdit ? "edit-boat show" : "edit-boat"}>
        <h2>Edit a boat</h2>
